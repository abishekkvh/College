...
#define PORT 7001
#define TITLE "Railway Reservation Server"
#define DATA_FILE "railway.dat"
#define LOG_FILE "railway.log"
#define HELP                                                                                       \
...
{
    db.services[0] =
        (struct Service){"Chennai Express", "Chennai", "Bengaluru", "06:00", 120, 120, 350};
    db.services[1] =
        (struct Service){"Coastal Express", "Chennai", "Madurai", "08:30", 100, 100, 280};
    db.services[2] =
        (struct Service){"Southern Express", "Bengaluru", "Madurai", "21:00", 80, 80, 450};
}

static void show_service(int i)
{
    struct Service *s = &db.services[i];
    reply("%d | %s | %s -> %s | %s | Seats: %d/%d | Fare: Rs %d\n", i + 1, s->name, s->origin,
          s->destination, s->timing, s->available, s->capacity, s->fare);
}
...
static int handle(int count, char **f)
{
    ...
    else if (count == 3 && strcmp(f[0], "search") == 0)
    {
        int found = 0;
        for (int i = 0; i < 3; i++)
        {
            if (!strcasecmp(f[1], db.services[i].origin) &&
                !strcasecmp(f[2], db.services[i].destination))
            {
                show_service(i);
                found = 1;
            }
